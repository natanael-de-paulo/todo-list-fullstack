...
class UpdateTaskController{
  constructor(private updateTaskService: ITasksServices<DataTaskToUpdateDTO, void>){}
  
  async handle(req: Request, res: Response){
    const { userId, taskId } = req.params
    const { name, status } = req.body
    const respose = await this.updateTaskService.execute({userId, taskId, name, status})
